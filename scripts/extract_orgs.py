...
def filter_github_organizations(urls):
    ...
    for url in urls:
        ...
        if len(parts) > 3:
            # Handle repository paths that include /tree/ or # segments
            org_name = parts[3]

            # Don't add names that are likely repositories (containing specific patterns)
            if "#" not in org_name and ".js" not in org_name and ".py" not in org_name:
