...
def get_vscode_repos(
    ...
    max_language_size=1000000,  # Added max_language_size
):
    """
    ...
    Args:
        ...
        max_language_size: The maximum size (in bytes) allowed for the language portion of the repository.
    ...
    while True:
        ...
        query = f"pushed:>{one_year_ago_str}"
...
def main():
    parser = argparse.ArgumentParser(description="Fetch popular VSCode repositories from GitHub.")
    ...
    parser.add_argument(
        "--max-language-size", type=int, default=1000000, help="Maximum language size in bytes (default: 1000000)"
    )  # Added argument
    ...
    repo_name_filter = None
    excludes_description = None
    required_in_name = "vscode"

    repos = get_vscode_repos(
        token,
        max_repos=args.max,
        ...
        max_language_size=args.max_language_size,  # Pass the argument
    )
